...
//
// Each PATCH request must contain a body of, at least, 5MB. If your server
// supports a different limit, you can adjust this value using
// S3Store.MinPartSize. If the size is smaller than this limit, then the body
// will be stored in a temporary object in S3. When a subsequent incoming PATCH
// request is received, the temporary object is fetched and prepended to the
// request body.
//
...
func (store S3Store) GetInfo(id string) (info tusd.FileInfo, err error) {
	...
	if err != nil {
		if !isAwsError(err, s3.ErrCodeNoSuchKey) && !isAwsError(err, "AccessDenied") && !isAwsError(err, "Forbidden"){
			return info, err
		}
